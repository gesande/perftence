...
public final class EstimatedTimeLeftBasedOnThroughput extends SummaryFieldImpl<String> {
    private final FieldFormatter fieldFormatter;

    private long invocationsLeft;
    private double currentThroughput;

    private EstimatedTimeLeftBasedOnThroughput(final FieldFormatter fieldFormatter,
            final FieldDefinition fieldDefinition) {
        super(fieldDefinition);
        this.fieldFormatter = fieldFormatter;
    }

    static EstimatedTimeLeftBasedOnThroughput create(FieldFormatter fieldFormatter, FieldAdjuster fieldAdjuster) {
        return new EstimatedTimeLeftBasedOnThroughput(fieldFormatter,
                SummaryFieldFactory.adjustedFieldName(Fields.EstimatedTimeLeft, fieldAdjuster));
    }

    public EstimatedTimeLeftBasedOnThroughput invocationsLeft(final long invocationsLeft) {
    ...
    @Override
    public String value() {
        return new StringBuilder().append(format(estimatedTimeLeft())).append(" (sec)").toString();
    }

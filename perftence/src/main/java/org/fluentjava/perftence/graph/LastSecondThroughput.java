...
import org.fluentjava.perftence.AsSynchronized;
import org.fluentjava.perftence.reporting.summary.ValueReporter;

public final class LastSecondThroughput implements ValueReporter<Double>, GraphWriterProvider {
    private final List<Double> throughputs;
    private final LineChartAdapterProvider<?, ?> linecharAdapterProvider;

    public LastSecondThroughput(final LineChartAdapterProvider<?, ?> linecharAdapterProvider) {
    ...
    public GraphWriter graphWriterFor(final String id) {
        return new GraphWriter() {
            ...
            private ImageData throughputData() {
                ...
                final ImageData imageData = ImageData.noStatistics(title, "Seconds",

...
public final class PerformanceRequirementsPojo implements PerformanceRequirements {
    ...
    public static class PerformanceRequirementsBuilder {
        ...
        public PerformanceRequirements build() {
            return newPerformanceRequirementsPojo(this.average, this.median, this.max, this.totalTime, this.throughput,
                    percentilesToArray());
        }

        private PercentileRequirement[] percentilesToArray() {
            return percentiles().toArray(new PercentileRequirement[percentiles().size()]);
        }

        private static PerformanceRequirements newPerformanceRequirementsPojo(final int average, final int median,
                final int max, final long totalTime, final int throughput, final PercentileRequirement[] percentiles) {
            return new PerformanceRequirementsPojo(average, median, max, totalTime, throughput, percentiles);
        }

        public static PerformanceRequirements noRequirements() {
            return newPerformanceRequirementsPojo(-1, -1, -1, -1, -1, new PercentileRequirement[0]);
        }
        ...
        private static PercentileRequirement newPercentileRequirement(final int percentage, final int value) {
    ...
    private PerformanceRequirementsPojo(final int average, final int median, final int max, final long totalTime,
            final int throughput, final PercentileRequirement[] percentileRequirements) {
        this.average = average;
        this.median = median;
        this.max = max;
        this.totalTime = totalTime;
    ...
    public PercentileRequirement[] percentileRequirements() {
        return Arrays.copyOf(this.percentileRequirements, this.percentileRequirements.length);
    }

    @Override
    public String toString() {
        return "PerformanceRequirementsPojo [average=" + average() + ", median=" + median() + ", max=" + max()
                + ", totalTime=" + totalTime() + ", throughput=" + throughput() + ", percentileRequirements="

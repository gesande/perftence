...
public class TestAgentUserStories extends AbstractMultiThreadedTest {
    ...
    public void sleepingAgentStoryWith10Agents() {
        agentBasedTest().agents(agents(10, new SleepingTestAgentFactory())).start();
    }

    @Test
    public void sleepingAgentStoryWith10AgentsWithCustomSummaryAppender() {
        agentBasedTest().agents(agents(10, new SleepingTestAgentFactory())).summaryAppender(summary -> {
    ...
    public void sleepingAgentStory() {
        agentBasedTest().agents(agents(1500, new SleepingTestAgentFactory())).latencyGraphForAll().start();
    }

    @Test
    public void sleepingAgentStoryWithSpecifiedLatencyGraph() {
        agentBasedTest("sleepingAgentStoryWithSpecifiedLatencyGraph").agents(agents(10, new SleepingTestAgentFactory()))
                .latencyGraphFor(SleepingTestCategory.AliveAgent).start();
    }

    @Test
    public void sleepingAgentWithFixedAmountOfWorkers() {
        agentBasedTest().workers(1000).agents(agents(1000, new SleepingTestAgentFactory())).latencyGraphForAll()
                .start();
    }

    @Test
    public void sleepingAgentStoryWithNoInvocationGraphsNorThreadsRunningCurrentTasksNorTaskScheduleDifferencies() {
        agentBasedTest().noInvocationGraph().noThreadsRunningCurrentTasks().noTaskScheduleDifferencies()
                .agents(agents(10, new SleepingTestAgentFactory(), 1100)).start();
    }
    ...
    private class SleepingTestAgent implements TestAgent {
        ...
        public SleepingTestAgent(final int id, final int sleepValue) {
            ...
            this.firstTask = newSleepingTask(sleepValue(), evaluateIfNextTaskIsNeeded());
        }
    ...
    private class SleepingTask implements TestTask {
        ...
        public String toString() {
            return "SleepingTask [sleep=" + sleep() + ", category = " + category() + ", nextTask=" + hasNextTask()
                    + "]";
        }
        ...
        private SleepingTask(final int sleep, final boolean nextTask, final SleepingTestCategory category) {
            this.sleep = sleep;
    ...
    private static SleepingTestCategory randomCategory(final SleepingTestCategory[] values) {

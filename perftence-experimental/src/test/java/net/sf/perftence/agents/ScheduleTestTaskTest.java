...
public class ScheduleTestTaskTest {
	...
	public void scheduleToFuture() throws InterruptedException {
		...
		final ScheduledTestTask task = new ScheduledTestTask(
				task(inMillis(250)), scheduled);
	...
	private static String timeToRun(final ScheduledTestTask task,
			final long scheduled, final long now) {
		return "It should have been time to run! Now =" + now
				+ " scheduled time=" + scheduled + ", timeItWasScheduled="
				+ task.timeItWasScheduledToBeRun();

	}

	private static String notTimeToRun(final ScheduledTestTask task,
			final long scheduled, final long now) {
		return "It should not have been time to run! Now =" + now
				+ " scheduled time=" + scheduled + ", timeItWasScheduled="
				+ task.timeItWasScheduledToBeRun();
	}

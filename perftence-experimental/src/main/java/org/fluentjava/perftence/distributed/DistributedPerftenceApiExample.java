...
import org.fluentjava.perftence.LoggingTestFailure;
import org.fluentjava.perftence.fluent.PerformanceRequirementsPojo;
...
public class DistributedPerftenceApiExample {
    private final static Logger LOG = LoggerFactory.getLogger(DistributedPerftenceApiExample.class);
    ...
    public void exampleOfDistributedTest() throws MalformedURLException {
        ...
        final DistributedPerftenceApi api = new DistributedPerftenceApi(new LoggingTestFailure(), reporterFactory)
                .reportingThreads(1).reportingLatenciesTo(new URL("http://localhost:9001/report/latency"));
        api.test(id()).setup(setup().threads(10).invocations(100).build()).executable(new Executable() {
            @Override
    ...
    private static DistributedLatencyReporterFactory newReporterFactory() {
        final DistributedLatencyReporterFactory reporterFactory = new DistributedLatencyReporterFactory() {
            ...
            @Override
            public RemoteLatencyReporter forRemoteReporting(final String id, final URL reportsTo) {
                return new RemoteLatencyReporter() {

                    @Override
                    public void latency(final int latency) {
                        LOG.info("reporting for " + id + " latency " + latency + " remotely "

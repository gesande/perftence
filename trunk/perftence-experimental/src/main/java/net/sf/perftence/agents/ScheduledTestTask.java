...
final class ScheduledTestTask implements Comparable<ScheduledTestTask> {
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + (int) (this.scheduleTime ^ (this.scheduleTime >>> 32));
        result = prime * result + (int) (this.scheduledRunTimeInNanos ^ (this.scheduledRunTimeInNanos >>> 32));
    ...
    public ScheduledTestTask(final TestTask task, final long scheduleTime) {
        ...
        this.scheduledRunTimeInNanos = TimeSpecificationFactory.toNanos(task.when()) + scheduleTime;
    }
    ...
    public int compareTo(final ScheduledTestTask o) {
        ...
        return compare == 0 ? this.scheduleTime().compareTo(o.scheduleTime()) : compare;
    }

    private static int compare(final TestTask o1, final TestTask o2) {
        final Time when = o1.when();
        final Time when2 = o2.when();
        final Long time1 = TimeUnit.NANOSECONDS.convert(when.time(), when.timeUnit());
        final Long time2 = TimeUnit.NANOSECONDS.convert(when2.time(), when2.timeUnit());

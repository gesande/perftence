...
final class TaskProvider implements TaskSchedulingStatisticsProvider {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(TaskProvider.class);
    ...
    public TaskProvider(final Time time) {
        this.waitTimeout = TimeUnit.MILLISECONDS.convert(time.time(), time.timeUnit());
        this.scheduledTasks = new TreeSet<>();
    }

    public ScheduledTestTask waitForNextScheduledTask() throws InterruptedException {
        synchronized (this.scheduledTasks) {
            ...
            if (!remove) {
                log().warn("Unable to remove the task from the scheduled tasks!");
            }
    ...
    private ScheduledTestTask nextScheduledTask() {
        return this.scheduledTasks.isEmpty() ? null : this.scheduledTasks.first();
    }
    ...
    public void schedule(final TestTask task) throws ScheduleFailedException {
        synchronized (this.scheduledTasks) {
            final ScheduledTestTask scheduled = new ScheduledTestTask(task, System.nanoTime());
    ...
    public Time lastTaskScheduledToBeRun() {
        return hasScheduledTasks()
                ? TimeSpecificationFactory.inNanos(this.scheduledTasks.last().timeItWasScheduledToBeRun())

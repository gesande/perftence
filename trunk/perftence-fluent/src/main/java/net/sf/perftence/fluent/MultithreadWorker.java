...
public final class MultithreadWorker implements Startable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(MultithreadWorker.class);
    ...
    public MultithreadWorker(final TestRuntimeReporter reporter, final InvocationRunner runner,
            final PerformanceTestSetup setUp, final LatencyProvider latencyProvider,
            final AllowedExceptions allowedExceptions, final AllowedErrors allowedErrors,
            final PerformanceRequirementValidator requirementValidator, final TestSummaryLogger overAllSummaryLogger,
            final TestSummaryLogger intermediateSummaryLogger, final LatencyFactory latencyFactory,
            final AllowedExceptionOrErrorOccurredMessageBuilder allowedExceptionOrErrorOccurredMessageBuilder,
            final PerfTestFailureFactory perfTestFailureFactory, final SummaryConsumer summaryConsumer) {
    ...
    private Invocation[] createRunnables(final Executable original) {
        ...
        throw newPerfTestFailure("Invalid test setup " + setUp().toString() + "...check it out!");
    }

    private Invocation[] threadBased(final PerformanceTestSetup setUp, final MeasurableExecutable measurable) {
        log().info("This seems to be thread based test");
        final int invocationsPerThread = invocationsPerThread(setUp);
        return invocationsFor(measurable, invocationsPerThread, extraInvocations(setUp, invocationsPerThread));
    }
    ...
    private int extraInvocations(final PerformanceTestSetup setUp, final int invocationsPerThread) {
        return setUp.invocations() - (invocationsPerThread * totalThreads());
    }

    private Invocation[] invocationsFor(final MeasurableExecutable adaptedExecutable, final int invocationsPerThread,
            final int extraInvocations) {
        final List<Invocation> invocations = new ArrayList<>();
        final Invocation[] invocationList = invocationBased(adaptedExecutable, invocationsPerThread, totalThreads());
    ...
    private void checkForExtraThreads(final MeasurableExecutable adaptedExecutable, final int extraInvocations,
    ...
    private void addExtraThreads(final MeasurableExecutable adaptedExecutable, final int extraInvocations,
            final List<Invocation> invocations) {
        informAboutExtraThread(extraInvocations);
        final Invocation[] extra = invocationBased(adaptedExecutable, extraInvocations, 1);
    ...
    private static void informAboutExtraThread(final int extraInvocations) {
        log().info("Need to have extra thread with {} invocations.", extraInvocations);
    }

    private Invocation[] durationBased(final MeasurableExecutable executable, final int duration) {
    ...
    final class DurationBased extends Invocation {
        ...
        public DurationBased(final MeasurableExecutable executable, final int duration) {
    ...
    private String allowedExceptionOccurredMessage(final Exception cause) {
        return this.allowedExceptionOrErrorOccurredMessageBuilder.allowedExceptionOccurredMessage(cause, id());
    }

    private String allowedErrorOccurredMessage(final Error cause) {
        return this.allowedExceptionOrErrorOccurredMessageBuilder.allowedErrorOccurredMessage(cause, id());
    }
    ...
    private void reportThroughput() {
        reportThroughput(latencyProvider().currentDuration(), latencyProvider().currentThroughput());
    }

    private void reportThroughput(final long currentDuration, final double currentThroughput) {
    ...
    private Invocation[] invocationBased(final MeasurableExecutable executable, final int invocationsPerThread,
            final int threads) {
    ...
    final class InvocationBased extends Invocation {
        ...
        public InvocationBased(final MeasurableExecutable executable, final int invocationsPerThread) {
            this.executable = executable;
    ...
    private void newSummary() {
        ...
        reporter().summary(id(), latencyProvider().duration(), latencyProvider().sampleCount(),
        ...
        scheduleTimer(new TimerSpec() {
            ...
            public TimerTask task() {
                return new TimerTask() {
                    @Override
                    public void run() {
                        if (latencyProvider().currentDuration() > setUp().duration() + Duration.seconds(5)) {
                            log().error(
    ...
    public MultithreadWorker customInvocationReporters(final CustomInvocationReporter... customReporters) {
    ...
    public MultithreadWorker customFailureReporter(CustomFailureReporter... customFailureReporters) {

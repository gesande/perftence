...
public final class EstimatedSamplesField extends SummaryFieldImpl<String> {
    ...
    static EstimatedSamplesField create(final FieldAdjuster fieldAdjuster) {
        return new EstimatedSamplesField(SummaryFieldFactory.adjustedFieldName(Fields.Samples, fieldAdjuster));
    }
    ...
    @Override
    public String value() {
        return new StringBuilder().append(sampleCount()).append("/").append(estimatedInvocations())
                .append(sampleCount() < estimatedInvocations() ? " (estimated)" : "").toString();
    }
}
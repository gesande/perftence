...
public abstract class AbstractTestRunner extends BlockJUnit4ClassRunner
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(AbstractTestRunner.class);
    ...
    private void tryToInjectFailureNotifier(final Class<?> clazz) {
        try {
            injectFailureNotifier(clazz.getMethod(failureNotifierMethod(),
                    TestFailureNotifier.class));
    ...
    private static String notAbleToInjectTestFailureNotifier() {
        return "Not able to inject failure notifier {}. "
                ...
                + "To be able to define performance tests fluently, "
                + "please extend your test class from "
    ...
    private void runTestMethod(final FrameworkMethod method,
            final EachTestNotifier eachNotifier) {
    ...
    private EachTestNotifier makeNotifier(final FrameworkMethod method,
            final RunNotifier notifier) {
     ...
     * executed.
     */
    protected abstract void processTestMethods(
            final List<FrameworkMethod> methods);

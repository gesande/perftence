...
public final class LastSecondIntermediateStatisticsProvider implements CustomIntermediateSummaryProvider {
    ...
    public LastSecondIntermediateStatisticsProvider(final AdjustedFieldBuilder fieldBuilder,
            final RuntimeStatisticsProvider statistics, final ValueReporter<Double> lastSecondThroughput) {
    ...
    public void provideIntermediateSummary(final IntermediateSummary summary) {
        ...
        summary.field(fieldBuilder().field("samples:", statistics().sampleCount()));
        summary.field(fieldBuilder().field("max:", statistics().maxLatency()));
        summary.field(fieldBuilder().field("average:", statistics().averageLatency()).asFormatted());
        summary.field(fieldBuilder().field("median:", statistics().median()));
        summary.field(fieldBuilder().field("95 percentile:", statistics().percentileLatency(95)));
        final double currentThroughput = statistics().currentThroughput();
        reportThroughput(currentThroughput);
        summary.field(fieldBuilder().field("throughput:", currentThroughput).asFormatted());
    }

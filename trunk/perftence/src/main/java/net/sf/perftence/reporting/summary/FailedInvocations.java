...
public final class FailedInvocations implements CustomSummaryProvider<HtmlSummary>, CustomIntermediateSummaryProvider {
    ...
    FailedInvocations(final AdjustedFieldBuilder fieldBuilder, final DefaultDoubleFormatter formatter) {
    ...
    public void provideSummary(final Summary<HtmlSummary> summary) {
        summary.endOfLine();
        summary.text("Invocation success rate : ").text(format(successRate())).text(" %").endOfLine();
        summary.endOfLine();
        ...
        summary.text("Failed invocations : ").text(Long.toString(failed())).endOfLine();
        summary.endOfLine();
        if (!uniqueExceptions().isEmpty()) {
            summary.text("Exception summary:").endOfLine();
            for (final String exceptionClass : uniqueExceptions()) {
                summary.text(exceptionClass).text(", count = ").text(exceptionCount(exceptionClass).toString())
                        .endOfLine();
            }
        }
    }
    ...
    private AdjustedField<Integer> exceptionSummaryField(final String exceptionClass) {
        return fieldBuilder().field(exceptionClass + ":", exceptionCount(exceptionClass));
    }
    ...
    public CustomSummaryProvider<HtmlSummary> invocations(final long invocationCount) {

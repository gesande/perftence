...
final class DefaultTestRuntimeReporter implements TestRuntimeReporter {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultTestRuntimeReporter.class);
    ...
    public DefaultTestRuntimeReporter(final InvocationStorage invocationStorage,
            final ThroughputStorage throughputStorage, final ImageFactory imageFactory, final int threadCount,
            final int duration, final FrequencyStorage frequencyStorage,
            final Collection<SummaryAppender> summaryAppenders, final boolean includeInvocationGraph,
            final Collection<GraphWriter> graphWriters, final StatisticsSummaryProvider<HtmlSummary> statisticsProvider,
            final FailedInvocations failedInvocations, final TestReport testReport) {
    ...
    private StringBuilder createSummary(final String id, final long elapsedTime, final long invocationCount) {
    ...
    private boolean writeFrequencyChart(final String id) {
        if (frequencyStorage().hasSamples()) {
            imageFactory().createXYLineChart(id, frequencyStorage().imageData());
            return true;
        }
        return false;
    }
    ...
    private void appendThroughputChart(final String id, final HtmlSummary summary) {
    ...
    private void appendInvocationChart(final String id, final HtmlSummary summary) {
    ...
    private void buildSummary(final String name, final long elapsedTime, final long invocationCount,
        ...
        statisticsSummaryProvider().elapsedTime(elapsedTime).invocations(invocationCount).provideSummary(summary);
        failedInvocations().invocations(invocationCount).provideSummary(summary);
        if (reportedLatencyBeingBelowOne()) {
            summary.note().text("Test reported about some invocations being less than one millisecond.").endOfLine()
                    .text("That is always calculated as one millisecond instead of zero.").endOfLine();
        }
        summary.endOfLine();
    ...
    public void summary(final String id, final long elapsedTime, final long sampleCount, final long startTime) {
        ...
        if (sampleCount == 0) {
            log().info("No need for summary because no invocations were reported for {}.", id);
            return;
        }

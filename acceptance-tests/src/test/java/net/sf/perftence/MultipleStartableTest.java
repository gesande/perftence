...
public class MultipleStartableTest extends AbstractMultiThreadedTest {
	...
	public void execute() throws Exception {
		test().startable(threadWorker(idOne(), 10, 100))
				.startable(threadWorker(idTwo(), 15, 100))
	...
	public void duration() throws Exception {
		test().startable(durationWorker(idOne(), 10))
				.startable(durationWorker(idTwo(), 10))
	...
	private MultithreadWorker threadWorker(final String id1, final int threads,
			final int invocations) throws Exception {
		return test(id1).setup(
				setup().threads(threads).invocations(invocations).build())

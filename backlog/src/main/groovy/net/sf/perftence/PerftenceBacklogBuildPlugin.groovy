...
class PerftenceBacklogBuildPlugin implements Plugin<Project>{
    ...
    public void apply(final Project project) {
        project.task("showBacklog",dependsOn: "classes") { Task task ->
            ...
            doLast {
                javaexec {
                    main = 'net.sf.perftence.backlog.PerftenceBacklogMain'
        ...
        project.task("waitingForImplementation",dependsOn: "classes") { Task task ->
            ...
            doLast {
                javaexec {
                    main = 'net.sf.perftence.backlog.BacklogWaitingForImplementation'
                    classpath  sourceSets.main.output.classesDir
                    classpath configurations.runtime
                }
            }
    ...
    private String changeLogArgsForJavaBacklog(Task task) {
        def String args = "svn diff -r ${task.project.properties.fromRevision}:${task.project.properties.toRevision} ${task.project.projectDir}/src/main/java/net/sf/perftence/backlog/PerftenceBacklog.java | grep 'done(' | grep '+' | sed 's/+                        //'"
        return args
    }

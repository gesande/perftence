...
public final class SummaryFieldFactoryForAgentBasedTests
	...
	public SummaryFieldFactoryForAgentBasedTests(
	...
	public SummaryField<Long> failedTasks(final long value) {
		return summaryFieldFactory().custom(Fields.FailedTasks, Long.class)
				.value(value).build();
	}

	public SummaryField<Integer> scheduleTasks(final int value) {
		return summaryFieldFactory()
				.custom(Fields.ScheduledTasks, Integer.class).value(value)
				.build();
	}

	public SummaryField<Long> finishedTasks(final long value) {
		return summaryFieldFactory().custom(Fields.FinishedTasks, Long.class)
				.value(value).build();
	}

	public SummaryField<Integer> threadsRunningCurrentTasks(final int value) {
		return summaryFieldFactory()
				.custom(Fields.ThreadsRunningTasks, Integer.class).value(value)
				.build();
	}

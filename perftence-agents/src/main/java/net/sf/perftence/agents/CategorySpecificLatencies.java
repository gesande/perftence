...
final class CategorySpecificLatencies {
    private final static Logger LOG = LoggerFactory.getLogger(CategorySpecificLatencies.class);
    ...
    public CategorySpecificLatencies(final CategorySpecificReporterFactory reporterFactory,
    ...
    public void startAdapters() {
        ...
        for (InvocationReporterAdapter adapter : categorySpecificReporters().values()) {
            adapter.start();
        }
    ...
    public void summaryTime() {
        if (hasCategorySpecificReporters()) {
            ...
            for (final InvocationReporterAdapter adapter : categorySpecificReporters().values()) {
                adapter.summary();
            }
    ...
    public void register(final TestTaskCategory category, final InvocationReporterAdapter reporter) {
        categorySpecificReporters().put(category, reporter);
    }

    public synchronized void reportLatencyFor(final int latency, final TestTaskCategory category) {
    ...
    private static void reportLatency(final InvocationReporterAdapter adapter, final int latency) {
        adapter.latency(latency);
    }

    private InvocationReporterAdapter newCategorySpecificReporter(final TestTaskCategory category) {
        return registerFor(category, reporterFactory().adapterFor(invocationReporter(), category));
    }
    ...
    private InvocationReporterAdapter registerFor(final TestTaskCategory category,
    ...
    private static void reportFailure(final InvocationReporterAdapter reporter, final Throwable cause) {

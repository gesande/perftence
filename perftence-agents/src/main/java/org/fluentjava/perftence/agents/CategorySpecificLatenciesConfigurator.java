...
class CategorySpecificLatenciesConfigurator {
    private final static Logger LOG = LoggerFactory.getLogger(CategorySpecificLatenciesConfigurator.class);
    ...
    public CategorySpecificLatenciesConfigurator(final CategorySpecificLatencies latencies,
    ...
    private void newCategorySpecificReporter(final TestTaskCategory category) {
        categorySpecificLatencies().register(category,
                reporterFactory().adapterFor(invocationReporterFactory(), category));
    }

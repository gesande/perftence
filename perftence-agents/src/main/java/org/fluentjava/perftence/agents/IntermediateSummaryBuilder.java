...
final class IntermediateSummaryBuilder extends AbstractSummaryBuilder {
    ...
    IntermediateSummaryBuilder(final RuntimeStatisticsProvider statisticsProvider, final ActiveThreads activeThreads,
            final TaskSchedulingStatisticsProvider scheduledTasks, final TestFailureNotifierDecorator failureNotifier,
    ...
    protected void fields(final TestSummary summary) {
        ...
        summary.field(throughput(statistics().currentThroughput()).asFormatted());
        summary.field(executionTime(statistics().currentDuration()));
        summary.field(lastTaskToBeRun(scheduledTasks().lastTaskScheduledToBeRun()));
